...
7. **Cryptographic Audit Chain & data integrety**:
   - SHA-256 Document Fingerprint: Every upload tender document is instantly hashed at the binary level using SHA-256 upon ingestion to the create a unique cryptographic fingerprint.
   - Tamper-Evident Linked Ledger: Each compliance report stores both its current 'file_hash' and 'previous_file_hash' of the preceeding record, establishing an immutable cryptographic chain.
   - Audit Verifiaction Interface: Built-in UI inspect modal ('ReportModal.jsx') and evaluation deck allow procurement officiers to review document hashes, verify audit chain continuity, and detect manual database tampering.
